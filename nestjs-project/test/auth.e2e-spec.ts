...
/**
 * `res.body` do supertest é `any`, e cada acesso direto propaga esse `any`
 * para a asserção inteira. Estes tipos descrevem os corpos que os cenários
 * abaixo realmente inspecionam, para que o compilador continue conferindo.
 */
interface ErrorBody {
  error: string;
...
describe('Auth (e2e)', () => {
  ...
  describe('POST /auth/register', () => {
    it('returns 201 with { id, email } on valid registration', async () => {
      ...
      expect((res.body as RegisteredUserBody).id).toBeDefined();
      expect((res.body as RegisteredUserBody).email).toBe('user@example.com');
    });
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      expect((res.body as JwtPayloadBody).sub).toBeDefined();
      expect((res.body as JwtPayloadBody).email).toBe('me@example.com');
    });
