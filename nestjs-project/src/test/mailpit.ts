...
/** Endereço como o Mailpit devolve em `To`, `From` e afins. */
export interface MailpitAddress {
  Address: string;
  Name: string;
}

/** Item da listagem `GET /api/v1/messages`. */
...
/** Corpo de `GET /api/v1/message/{id}`, com o conteúdo renderizado. */
export interface MailpitMessage extends MailpitMessageSummary {
  HTML: string;
  Text: string;
}

// Tipar o retorno aqui, e não em cada spec: um `any` nesta fronteira se
// propaga para toda asserção que toca a mensagem, e era a origem da maior
// parte dos erros de `no-unsafe-member-access` na suíte de e-mail.
